...
# ==============================================================================
# NOTE: This version of the MCP server replaces the `get_cases_by_project` tool
# with `get_cases_by_cohort_description`, a tool to get arbitrary case cohorts
# (using the GDC's `/cases` endpoint) through natural language, via cohort copilot
# ==============================================================================

# bring in some cohort copilot utilities
sys.path.append(os.path.join(os.path.dirname(__file__), "cohort-copilot"))
from cohort_copilot import generate_filter


@mcp.tool()
def get_cases_by_cohort_description(cohort_description: CohortDescription) -> CaseSetId:
    """
    ...
    # NOTE: in this demo, we rely on a trained GPT2 model for cohort copilot, which is small enough to run on CPU
    # TODO: cohort copilot is undergoing major revisions and this will likely need to be externalized
    filter_str = generate_filter(cohort_description)

    # NOTE: there's not a great way in the current tool design to surface the cohort description to the user, for now just log it
    print(
        f"Generated Cohort Filter:\n{filter_str}",
        file=sys.stderr,
    )
...
TOOL_TO_CACHE_ID_PATTERN = {
    ...
    "get_cases_by_cohort_description": re.compile(
        r"Cases-Cohort-(?P<cohort_description>.+)",
    ),
}
